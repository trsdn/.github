...
| `packages/` | Versioned agent packages other repositories install with APM and pin to a tag of this repository. Keep them free of per-repository content. `apple-hig-review`, `performance-review`, `doc-staleness-reviewer` and `site-content-reviewer` are credential-free and read-only; `frontend-designer` writes files (never commits) and `repo-assessor` needs the operator's own `gh` write access to file issues and never writes a conformance record itself. |
